...
// void setup() {
//     Serial.begin(921600);  // Debugging over USB Serial
//     mySerial.begin(115200, SERIAL_8N1, 12, 13); // RX = GPIO12, TX = GPIO13
// }
...
//         float distance, yaw, pos_x, pos_y, velocity;
//         if (sscanf(receivedData.c_str(), "<D:%f,Y:%f,X:%f,Y:%f,V:%f>", 
//                 &distance, &yaw, &pos_x, &pos_y, &velocity) == 5) {

//             Serial.printf("Distance: %.2f m, Yaw: %.2f deg, X: %.2f m, Y: %.2f m, Velocity: %.2f m/s\n",
//                         distance, yaw, pos_x, pos_y, velocity);
            
//             // Use these values for path planning & obstacle avoidance
//         }
